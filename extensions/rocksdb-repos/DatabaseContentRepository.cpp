...
bool DatabaseContentRepository::initialize(
  ...
  if (status.ok()) {
    logger_->log_debug("NiFi Content DB Repository database open %s success",
                       directory_);
    is_valid_ = true;
  } else {
    logger_->log_error("NiFi Content DB Repository database open %s fail",
                       directory_);
    is_valid_ = false;
  }
  return is_valid_;
...
std::shared_ptr<io::BaseStream> DatabaseContentRepository::write(
...
std::shared_ptr<io::BaseStream> DatabaseContentRepository::read(
...
bool DatabaseContentRepository::exists(
    const std::shared_ptr<minifi::ResourceClaim> &streamId) {
  std::string value;
  rocksdb::Status status;
  status =
      db_->Get(rocksdb::ReadOptions(), streamId->getContentFullPath(), &value);
  if (status.ok()) {
...
bool DatabaseContentRepository::remove(
    const std::shared_ptr<minifi::ResourceClaim> &claim) {
